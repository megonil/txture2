 ...
 * Ported to C from Kurt Spencer's java implementation by Stephen M.
 * Cameron
 */
...
simplex_seed (int64_t seed, struct osn_context** ctx)
{
	...
	*ctx = (struct osn_context*) malloc (sizeof (**ctx));
	...
	uint64_t seedU = seed;
	for (i = 0; i < 256; i++) source[i] = (int16_t) i;
...
static OSNFLOAT
open_simplex_noise2 (const struct osn_context* ctx, OSNFLOAT x, OSNFLOAT y)
{
